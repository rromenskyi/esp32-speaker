...
"function send(files){if(!files.length){up.textContent='';bar.style.width=0;load();return}"
"const f=files.shift(),x=new XMLHttpRequest();up.textContent='Uploading '+f.name;"
"x.open('POST','/music/upload?name='+encodeURIComponent(f.name));const t=localStorage.tok;if(t)x.setRequestHeader('X-Token',t);"
"x.upload.onprogress=e=>bar.style.width=(e.loaded/e.total*100)+'%';"
"x.onload=()=>{if(x.status!=200)alert(f.name+': '+x.responseText);send(files)};"
"x.onerror=()=>{alert(f.name+': upload error');send(files)};x.send(f)}"
